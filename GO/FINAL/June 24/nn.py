...
class layer():
    ...
    def __init__(self, name='layer', shape=None, opt='adamax', param = 'he', trainable = True, mode = 'valid'):
        self.name = name
...
class linear(layer):
    def forward(self,x):
        ...
        if self.reshape:
            log.debug('nn.linear:-input shape {} is to be flattened for linear layer'.format(x.shape))
            self.x = self.x.reshape([-1])
            log.debug('nn.linear:- input shape {} is flattened to shape {}'.format(x.shape, self.x.shape))
...
class convolve3d(layer):          
    def forward(self,x):
        ...
        #sanity check
        if x.ndim == 2:  
            log.debug('nn.convolve3d:- Got input dim 2, reshaped to expected dim 3')
            self.x = np.reshape(self.x, [-1,np.shape(x)[0], np.shape(x)[1]])
        if self.x.ndim != 3:
            log.critical('Input dim {}, Expected input dim 3'.format(x.ndim))
...
class temp1(layer):
    def forward(self,x):
        self.x = x.copy()
        self.y = np.where(self.x==2,-1,self.x)
        return self.y
    def backward(self,dy):
        pass
    def update(self):
