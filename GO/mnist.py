...
# index which has not been used
index = tl.index('data/mnist/index.npy')
cwd = str(os.getcwd())
os.system('print ' + str(sys.argv[0]) + ' >> data/mnist/'+ str(sys.argv[0]) + str(index) +'.py')
...
# Number of epochs
for i in range(1):
    acc = model1.model((tni,tnl))
    tl.cprint('\n TicTac:mnist:- Training accuracy = {}, epoch ={}\n'.format(np.mean(acc),i), color='blue')
    #W = [conv1.w, add1.w, conv2.w, add2.w, lin.w]
    Acc = model1.model((tti,ttl), update=False) 
